...
const MarkerPopup: React.FC<MarkerPopupProps> = ({ market }) => {
  ...
  return (
    <div className="market-popup w-64">
      ...
      <h3 className="font-display text-lg font-bold mb-1 text-primary-700">{market.name}</h3>
      <p className="text-gray-600 text-xs mb-2">{market.address}, {market.city}, {market.state}</p>
      
      <p className={`text-sm font-medium mb-1 ${
        getTodayHours().includes('Closed') ? 'text-earth-700' : 'text-primary-600'
      }`}>
        {getTodayHours()}
      </p>
      
      {nextOpen && (
        <p className="text-xs text-earth-600 mb-2">{nextOpen}</p>
      )}
