...
const ParticleBackground: React.FC = () => {
  ...
  useEffect(() => {
      ...
      for (let row = 0; row < gridRows && count < particleCount; row++) {
        for (let col = 0; col < gridCols && count < particleCount; col++) {
          ...
          particles.current.push({
            ...
            shape: 'leaf',
            rotation: Math.atan2(windDirection.y, windDirection.x) + (Math.random() * Math.PI / 2 - Math.PI / 4),
    ...
    const animate = () => {
      ...
      particles.current.forEach((particle) => {
        ...
        drawLeaf(ctx, particle.x, particle.y, particle.size, particle.rotation, particle.color);
  ...
  return (
    <canvas
      ...
      style={{ 
        background: 'linear-gradient(135deg, oklch(0.65 0.21 148) 0%, #5b8c56 100%)'
      }}
    />
  );
};
