...
import { Product, Month } from '../../lib/types';
...
const groupProductsByMonth = (products: Product[]) => {
  ...
  const productsByMonth: Record<Month, Product[]> = {} as Record<Month, Product[]>;
...
const SeasonalCalendar: React.FC<SeasonalCalendarProps> = ({ className = '' }) => {
  const [selectedMonth, setSelectedMonth] = useState<Month>(getCurrentMonth());
  const [productsByMonth, setProductsByMonth] = useState<Record<Month, Product[]>>({} as Record<Month, Product[]>);
  ...
  if (!isLoaded) {
    return <div className="p-8 text-center">Loading seasonal information...</div>;
  }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden ${className}`}>
      ...
      <div className="p-6 border-b border-earth-200">
        <h3 className="font-display text-xl font-bold mb-4">In Season This Month</h3>
      ...
      <div className="p-6">
        ...
        {productsByMonth[selectedMonth].length === 0 ? (
          <div className="text-center py-12 bg-earth-50 rounded-lg">
            <p className="text-gray-500">No products in season for {selectedMonth}.</p>
            <p className="text-sm text-gray-400 mt-2">Try selecting a different month.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {productsByMonth[selectedMonth].map((product) => (
              ...
              >
                ...
                <div className="p-4">
                  <h4 className="font-display text-lg font-bold mb-1">{product.name}</h4>
      ...
      <div className="bg-primary-50 p-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm">
          <h4 className="font-medium text-lg mb-2">Making the Most of {selectedMonth} Produce</h4>
          <ul className="space-y-2">
            ...
            <li className="flex items-start">
              ...
              <span>Try preserving extra produce through freezing, canning, or drying</span>
            </li>
          </ul>
        </div>
      </div>
