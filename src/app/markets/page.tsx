...
const DynamicMapView = dynamic(() => import('../components/map/MapView'), {
  ssr: false,
  loading: () => <div className="h-[500px] bg-earth-100 rounded-lg flex items-center justify-center">Loading map...</div>
});
  ...
  return (
    <div className="min-h-screen bg-earth-50">
      <div className="container mx-auto px-4 py-8">
        <h1 className="font-display text-4xl font-bold text-primary-800 mb-8">Find Local Farmers Markets</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:order-2 lg:col-span-1">
            <div className="bg-white p-4 rounded-lg shadow-md h-[600px] overflow-y-auto">
              <h2 className="font-display text-2xl font-bold mb-4">Market Listings</h2>
              ...
              {filteredMarkets.length === 0 ? (
                ...
                <div className="space-y-4">
                  {filteredMarkets.map(market => (
                    <div 
                      key={market.id}
                      className={`cursor-pointer transition-all duration-300 border-l-4 ${selectedMarket?.id === market.id 
                        ? 'border-primary-500 bg-primary-50' 
                        : 'border-transparent hover:border-primary-200 hover:bg-earth-50'}`}
                      onClick={() => handleMarkerClick(market)}
                    >
                      <div className="p-3">
                        <h3 className="font-display font-bold text-lg">{market.name}</h3>
                        <p className="text-sm text-gray-600">{market.city}, {market.state}</p>
        ...
        {selectedMarket && (
          <div id="market-details" className="mt-8 bg-white p-6 rounded-lg shadow-md animate-fade-in">
            <h2 className="font-display text-3xl font-bold mb-4">{selectedMarket.name}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <p className="text-gray-700 mb-4">{selectedMarket.description}</p>
                <div className="mb-4">
                  <h3 className="font-display text-xl font-bold mb-2">Location</h3>
                  <p>{selectedMarket.address}</p>
                  <p>{selectedMarket.city}, {selectedMarket.state} {selectedMarket.zip}</p>
                </div>
                <div className="mb-4">
                  <h3 className="font-display text-xl font-bold mb-2">Market Hours</h3>
                ...
                <div>
                  <h3 className="font-display text-xl font-bold mb-2">Amenities</h3>
              ...
              <div>
                ...
                <div>
                  <h3 className="font-display text-xl font-bold mb-2">Reviews</h3>
                  {selectedMarket.reviews && selectedMarket.reviews.length > 0 ? (
                    <div className="space-y-3">
                      {selectedMarket.reviews.slice(0, 2).map(review => (
                        <div key={review.id} className="bg-earth-50 p-3 rounded-lg">
